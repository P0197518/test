package com.fujias.common.validation;

import static java.lang.annotation.ElementType.FIELD;
import static java.lang.annotation.RetentionPolicy.RUNTIME;

import java.lang.annotation.Documented;
import java.lang.annotation.Retention;
import java.lang.annotation.Target;

import javax.validation.Constraint;
import javax.validation.ConstraintValidator;
import javax.validation.ConstraintValidatorContext;
import javax.validation.Payload;
import javax.validation.ReportAsSingleViolation;

import org.hibernate.validator.constraints.CompositionType;
import org.hibernate.validator.constraints.ConstraintComposition;

import com.fujias.common.constants.MessageCodes;
import com.fujias.common.util.DateUtil;
import com.fujias.common.util.StringUtil;

/**
 * 日付文字列チェック
 *
 * @version 1.0
 * @author 陳強
 */
@Documented
@Constraint(validatedBy = {ValidateDateString.StrictDateStringValidator.class})
@Target(FIELD)
@Retention(RUNTIME)
@ConstraintComposition(CompositionType.OR)
@ReportAsSingleViolation
public @interface ValidateDateString {

    /**
     * メッセージパラメータを取得します。
     * 
     * @return メッセージパラメータ
     */
    String[] messageArgs();

    /**
     * メッセージを取得します。
     * 
     * @return メッセージ
     */
    String message() default MessageCodes.EC0005;

    /**
     * グループを取得します。
     * 
     * @return グループ
     */
    Class<?>[] groups() default {};

    /**
     * ペイロードを取得します。
     * 
     * @return ペイロード
     */
    Class<? extends Payload>[] payload() default {};

    /**
     * valueの日付パターンを取得します。
     * 
     * @return valueの日付パターン
     */
    String[] pattern() default {"yyyyMMdd"};

    /**
     * 日付文字列のみで構成されているかどうかチェックするバリデータです。
     * 
     */
    class StrictDateStringValidator implements ConstraintValidator<ValidateDateString, String> {

        /** 日付パターン */
        private String[] patterns;

        @Override
        public void initialize(ValidateDateString validStrictDateString) {
            this.patterns = validStrictDateString.pattern();
        }

        @Override
        public boolean isValid(String item, ConstraintValidatorContext context) {
            if (StringUtil.isEmpty(item)) {
                return true;
            }
            for (String pattern : patterns) {
                if (DateUtil.nullableParse(item, pattern).isPresent()) {
                    return true;
                }
            }
            return false;
        }
    }
}

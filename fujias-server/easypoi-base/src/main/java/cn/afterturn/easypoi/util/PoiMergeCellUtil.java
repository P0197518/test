 ...
 * 纵向合并单元格工具类
 *
 * @author JueYue 2015年6月21日 上午11:21:40
 */
public final class PoiMergeCellUtil {
     ...
     * @param delys
     */
    private static void handlerMergeCells(Integer index, int rowNum, String text, Map<Integer, MergeEntity> mergeDataMap, Sheet sheet, Cell cell,
                    int[] delys) {
        ...
        if (mergeEntity.getText().equals(text)) {
            for (int i = 0; i < delys.length; i++) {
                if (mergeEntity.getRelyList().get(i) == null
                                || !mergeEntity.getRelyList().get(i).equals(getCellNotNullText(cell, delys[i], rowNum))) {
                    return false;
                }
            }
            return true;
    ...
    private static String getCellNotNullText(Cell cell, int index, int rowNum) {
        ...
        if (cell.getRow().getCell(index) != null && StringUtils.isNotEmpty(PoiCellUtil.getCellValue(cell.getRow().getCell(index)))) {

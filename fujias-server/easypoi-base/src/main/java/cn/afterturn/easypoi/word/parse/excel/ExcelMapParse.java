 ...
 * 处理和生成Map 类型的数据变成表格
 *
 * @author JueYue 2014年8月9日 下午10:28:46
 */
public final class ExcelMapParse {
     ...
     * @param currentRun
     * @throws Exception
     * @author JueYue 2013-11-20
     */
    public static void addAnImage(ImageEntity obj, XWPFRun currentRun) {
        try {
            Object[] isAndType = PoiPublicUtil.getIsAndType(obj);
            String picId;
            picId = currentRun.getDocument().addPictureData((byte[]) isAndType[0], (Integer) isAndType[1]);
            if (obj.getLocationType() == ImageEntity.EMBED) {
                ((MyXWPFDocument) currentRun.getDocument()).createPicture(currentRun, picId,
                                currentRun.getDocument().getNextPicNameNumber((Integer) isAndType[1]), obj.getWidth(), obj.getHeight());
            } else if (obj.getLocationType() == ImageEntity.ABOVE) {
                ((MyXWPFDocument) currentRun.getDocument()).createPicture(currentRun, picId,
                                currentRun.getDocument().getNextPicNameNumber((Integer) isAndType[1]), obj.getWidth(), obj.getHeight(), false);
            } else if (obj.getLocationType() == ImageEntity.BEHIND) {
                ((MyXWPFDocument) currentRun.getDocument()).createPicture(currentRun, picId,
                                currentRun.getDocument().getNextPicNameNumber((Integer) isAndType[1]), obj.getWidth(), obj.getHeight(), true);
            }
    ...
    private static List<ExcelForEachParams> parseCurrentRowGetParamsEntity(XWPFTableRow currentRow) {
        ...
        for (int i = 0; i < cells.size(); i++) {
            ...
            text = cells.get(i).getText();
            param.setName(text == null ? "" : text.trim().replace(START_STR, EMPTY).replace(END_STR, EMPTY));
    ...
    public static void parseNextRowAndAddRow(XWPFTable table, int index, List<Object> list) throws Exception {
        ...
        listname = listname.replace(FOREACH_NOT_CREATE, EMPTY).replace(FOREACH_AND_SHIFT, EMPTY).replace(FOREACH, EMPTY).replace(START_STR, EMPTY);
        ...
        for (Object obj : list) {
            ...
            for (cellIndex = 0; cellIndex < currentRow.getTableCells().size(); cellIndex++) {
                ...
                if (val instanceof ImageEntity) {
                    ...
                    PoiWordStyleUtil.copyCellAndSetValue(tempCellList.get(cellIndex), currentRow.getTableCells().get(cellIndex), val.toString());
                }
            }
            ...
            for (; cellIndex < paramsList.size(); cellIndex++) {
                ...
                if (val instanceof ImageEntity) {
                    ...
                    PoiWordStyleUtil.copyCellAndSetValue(tempCellList.get(cellIndex), cell, val.toString());
                }
            }
        }

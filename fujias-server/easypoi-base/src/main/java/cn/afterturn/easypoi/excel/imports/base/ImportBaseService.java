...
public class ImportBaseService {
     ...
     * @throws Exception
     */
    public void addEntityToMap(String targetId, Field field, ExcelImportEntity excelEntity, Class<?> pojoClass, List<Method> getMethods,
    ...
    public void getAllExcelField(String targetId, Field[] fields, Map<String, ExcelImportEntity> excelParams,
                    List<ExcelCollectionParams> excelCollection, Class<?> pojoClass, List<Method> getMethods, ExcelEntity excelEntityAnn)
                    throws Exception {
        ...
        for (int i = 0; i < fields.length; i++) {
            ...
            if (PoiPublicUtil.isCollection(field.getType())) {
                ...
                if (excel.show() && StringUtils.isEmpty(excel.name())) {
                    throw new ExcelImportException("if use ExcelEntity ,name mus has value ,data: " + ReflectionToStringBuilder.toString(excel),
                                    ExcelImportEnum.PARAMETER_ERROR);
                }
                getAllExcelField(StringUtils.isNotEmpty(excel.id()) ? excel.id() : targetId, PoiPublicUtil.getClassFields(field.getType()),
                                excelParams, excelCollection, field.getType(), newMethods, excel);
            }
        }
    }
    ...
    public void getExcelFieldList(String targetId, Field[] fields, Class<?> pojoClass, Map<String, ExcelImportEntity> temp, List<Method> getMethods)
                    throws Exception {
        ExcelImportEntity excelEntity = null;
        for (int i = 0; i < fields.length; i++) {
            Field field = fields[i];
            ...
            if (PoiPublicUtil.isJavaClass(field) || field.getType().isEnum()) {
                ...
                getExcelFieldList(StringUtils.isNotEmpty(excel.id()) ? excel.id() : targetId, PoiPublicUtil.getClassFields(field.getType()),
    ...
    public void saveThisExcel(ImportParams params, Class<?> pojoClass, boolean isXSSFWorkbook, Workbook book) throws Exception {
        ...
        FileOutputStream fos = new FileOutputStream(path + File.separator + format.format(new Date()) + "_" + Math.round(Math.random() * 100000)

...
public class ExcelImportForkJoinWork extends RecursiveTask<ExcelImportResult> {
    ...
    public ExcelImportForkJoinWork(int startRow, int endRow, Sheet sheet, ImportParams params, Class<?> pojoClass, ExcelImportService importService,
                    String targetId, Map<Integer, String> titlemap, Map<String, ExcelImportEntity> excelParams) {
    ...
    protected ExcelImportResult compute() {
        ...
        if (length <= params.getCritical()) {
            ...
            ExcelImportForkJoinWork right = new ExcelImportForkJoinWork(startRow, middle, sheet, params, pojoClass, importService, targetId, titlemap,
                            excelParams);
            right.fork();
            ExcelImportForkJoinWork left = new ExcelImportForkJoinWork(middle + 1, endRow, sheet, params, pojoClass, importService, targetId,
                            titlemap, excelParams);
            left.fork();
            // 合并
            result = right.join();

...
public class PdfExportServer extends ExportCommonService {
    ...
    private void createCells(Table table, Object t, List<ExcelExportEntity> excelParams, int rowHeight) throws Exception {
     ...
     * @throws Exception
     */
    private void createListCells(Table table, Object obj, List<ExcelExportEntity> excelParams, int rowHeight) throws Exception {
        ExcelExportEntity entity;
        for (int k = 0, paramSize = excelParams.size(); k < paramSize; k++) {
            entity = excelParams.get(k);
    ...
    private void createHeaderAndTitle(PdfExportParams entity, Table table, List<ExcelExportEntity> excelParams) {
     ...
     * @param table
     */
    private int createTitleRow(PdfExportParams title, Table table, List<ExcelExportEntity> excelParams) {
    ...
    private Cell createStringCell(Table table, String text, ExcelExportEntity entity, int rowHeight, int colspan, int rowspan) {
    ...
    private Cell createStringCell(Table table, String text, ExcelExportEntity entity, int rowHeight) {
        Cell iCell = new Cell();
        iCell.add(new Paragraph(text));
        styler.setCellStyler(iCell, entity, text);
        iCell.setHeight((int) (rowHeight * 2.5));
    ...
    private Cell createImageCell(Table table, String text, ExcelExportEntity entity, int rowHeight, int rowSpan, int colSpan) {

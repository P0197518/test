...
public abstract class BaseExportService extends ExportCommonService {
     ...
     * 创建 最主要的 Cells
     */
    public int[] createCells(Drawing patriarch, int index, Object t, List<ExcelExportEntity> excelParams, Sheet sheet, Workbook workbook,
            ...
            for (int k = indexKey, paramSize = excelParams.size(); k < paramSize; k++) {
                ...
                if (entity.getList() != null) {
                    ...
                    if (entity.isHyperlink()) {
                        row.getCell(cellNum - 1).setHyperlink(
                                        dataHandler.getHyperlink(row.getSheet().getWorkbook().getCreationHelper(), t, entity.getName(), value));
                    }
                }
            }
    ...
    public void createImageCell(Cell cell, double height, String imagePath, byte[] data) throws Exception {
        ...
        if (type.equals(ExcelType.HSSF)) {
            ...
            anchor = new HSSFClientAnchor(10, 10, 1010, 245, (short) cell.getColumnIndex(), cell.getRow().getRowNum(),
                            (short) (cell.getColumnIndex()), cell.getRow().getRowNum());
        } else {
            anchor = new XSSFClientAnchor(10, 10, 1010, 245, (short) cell.getColumnIndex(), cell.getRow().getRowNum(),
                            (short) (cell.getColumnIndex()), cell.getRow().getRowNum());
        }
    ...
    public void createImageCell(Cell cell, double height, int rowspan, int colspan, String imagePath, byte[] data) throws Exception {
        ...
        if (type.equals(ExcelType.HSSF)) {
            anchor = new HSSFClientAnchor(10, 10, 1010, 245, (short) cell.getColumnIndex(), cell.getRow().getRowNum(),
                            (short) (cell.getColumnIndex() + colspan - 1), cell.getRow().getRowNum() + rowspan - 1);
        } else {
            anchor = new XSSFClientAnchor(10, 10, 1010, 245, (short) cell.getColumnIndex(), cell.getRow().getRowNum(),
                            (short) (cell.getColumnIndex() + colspan - 1), cell.getRow().getRowNum() + rowspan - 1);
        }
     ...
     * 创建List之后的各个Cells
     */
    public void createListCells(Drawing patriarch, int index, int cellNum, Object obj, List<ExcelExportEntity> excelParams, Sheet sheet,
        ...
        for (int k = 0, paramSize = excelParams.size(); k < paramSize; k++) {
            ...
            if (entity.getType() == BaseEntityTypeConstants.STRING_TYPE) {
                ...
                if (entity.isHyperlink()) {
                    row.getCell(cellNum - 1).setHyperlink(
                                    dataHandler.getHyperlink(row.getSheet().getWorkbook().getCreationHelper(), obj, entity.getName(), value));
                }
            } else if (entity.getType() == BaseEntityTypeConstants.DOUBLE_TYPE) {
                createDoubleCell(row, cellNum++, value == null ? "" : value.toString(),
                                index % 2 == 0 ? getStyles(false, entity) : getStyles(true, entity), entity);
    ...
    private void createCellComment(Row row, Cell cell, String text, ExcelExportEntity entity) {
        if (commentHandler != null) {
            String comment = entity == null || entity.getName().equals(text) || (entity.getGroupName() != null && entity.getGroupName().equals(text))
                            ? commentHandler.getComment(text)
                            : commentHandler.getComment(entity.getName(), text);
    ...
    private Comment getComment(Cell cell, String commentText, String author) {
        ...
        if (cell instanceof HSSFCell) {
            ...
            comment = cell.getSheet().createDrawingPatriarch()
                            .createCellComment(new HSSFClientAnchor(0, 0, 0, 0, (short) 3, 2, (short) 5, commentText.length() / 15 + 2));
            comment.setString(new HSSFRichTextString(commentText));
        } else {
            comment = cell.getSheet().createDrawingPatriarch()
                            .createCellComment(new XSSFClientAnchor(0, 0, 0, 0, (short) 3, 2, (short) 5, commentText.length() / 15 + 2));

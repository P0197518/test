...
public class ExcelToHtmlService {
                ...
                if (mergedRegionHelper.isNeedCreate(rowIndex, i)) {
                    ...
                    if (pictures.containsKey((rowIndex - 1) + "_" + i)) {
                        content = "<img src='data:image/" + PoiPublicUtil.getFileExtendName(pictures.get((rowIndex - 1) + "_" + i).getData())
                                        + ";base64," + getImageSrc(pictures.get((rowIndex - 1) + "_" + i)) + "' style='max-width:  "
                                        + getImageMaxWidth(mergedRegionHelper.getRowAndColSpan(rowIndex, i), i, sheet) + "px;' />";
                    }
                    if (mergedRegionHelper.isMergedRegion(rowIndex, i)) {
                        Integer[] rowAndColSpan = mergedRegionHelper.getRowAndColSpan(rowIndex, i);
                        out.format("    <td rowspan='%s' colspan='%s' class='%s' >%s</td>%n", rowAndColSpan[0], rowAndColSpan[1], styleName(style),
                                        content);
                    } else {
    ...
    private String getImageSrc(PictureData pictureData) {
        ...
        /*
         * String fileName = "pic" + Math.round(Math.random() * 100000000000L); fileName += "." + PoiPublicUtil.getFileExtendName(data); if (!imageCachePath.startsWith("/") && !imageCachePath.contains(":")) {
         * imageCachePath = FileUtilTest.getWebRootPath(imageCachePath); } File savefile = new File(imageCachePath + "/" + today); if (!savefile.exists()) { savefile.mkdirs(); } savefile = new
         * File(imageCachePath + "/" + today + "/" + fileName); FileOutputStream fos = null; try { fos = new FileOutputStream(savefile); fos.write(data); } catch (Exception e) { LOGGER.error(e.getMessage(),
         * e); } finally { try { fos.close(); } catch (Exception e) { LOGGER.error(e.getMessage(), e); } } return imageCachePath + "/" + today + "/" + fileName;
         */
    }

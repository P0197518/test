...
public class ParseWord07 {
    ...
    private Object checkThisTableIsNeedIterator(XWPFTableCell cell, Map<String, Object> map) throws Exception {
        ...
        if (text != null && text.contains(FOREACH) && text.startsWith(START_STR)) {
            text = text.replace(FOREACH_NOT_CREATE, EMPTY).replace(FOREACH_AND_SHIFT, EMPTY).replace(FOREACH, EMPTY).replace(START_STR, EMPTY);
     ...
     * @param paragraphs
     * @param map
     * @author JueYue 2013-11-17
     */
    private void parseAllParagraph(List<XWPFParagraph> paragraphs, Map<String, Object> map) throws Exception {

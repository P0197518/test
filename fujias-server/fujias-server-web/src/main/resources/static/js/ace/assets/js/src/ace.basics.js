...
(function($ , undefined) {
	...
	ace.demo.functions.bindClickToElement = function() {
        var index = 0;
	    $('.sidebar a').each(
	        function() {
	            ...
	            if(href && href != null && href != "" && href != "#") {
                    ...
                    if(href.match(/^page\//)) {
                        // 如果是html
                        realPath = basePath + '/template/ace/html/' + href.replace(/^page\//, '')+'.html';
                    } else {
                        realPath = basePath + '/' + href.replace(/^\.\.\//, '');
                    }
                    ...
                    if(span.length > 0) {
                        ...
                        text = $(this).contents().filter(function() { return this.nodeType === 3; }).text(); 
                    }
                    $(this).attr("title", text);
                    $(this).click(function() {
                        showTab({id:'tab' + $(this).attr("index"), text:$(this).attr("title"), url:realPath});
                        });
                    index++;
	            }
	        }
...
//		}
//		
//		var isJsp = function(){try{return basePath,true}catch(e){}}();
//
...
//			 
//			 'default_url': 'action/index',//default hash
...
//					if(hash.match(/^page\//)) {
//						// 如果是html
//						var returnPath = basePath + 'template/ace/html/ajax/content/'+hash.replace(/^page\//, '')+'.html';
//						return returnPath;
//					}
//					var path2 = hash.replace(/(\.\.\/)/, '').replace(/(\.action)/, '');
//					var path3 = path2.split("?").length;
//					if('action/index' == path2) {
//						return basePath + "jsp/navigation/home.jsp";
//					}
//					if (path3 > 1) {
//						var path4 = path2.split("?");
//						return basePath + path4[0] + ".action?" + path4[1];
//					} else {
//						return basePath + path2 + ".action";
//					}
//				} else {
//					if(path.match(/(\/ajax\/)(index\.html)?/)) {
//						var returnPath = path.replace(/(\/ajax\/)(index\.html)?/, '/ajax/content/'+hash.replace(/^page\//, '')+'.html');
//						return returnPath;
//					}
//				}
...
ace.parse = function() {
	...
	for(var i = 0; i < menubars.length; i++) {
		...
		var idField = menubar.attr("idField");
		var parentField = menubar.attr("parentField");
		$.getJSON(menubar.attr("url"), function(data) {
				var rootItems = new Array();
				// 得到根节点
				for(var i = 0; i < data.length; i++) {
					var isRoot = function(){try{if(eval("data[i]." + parentField)){return false;} else return true;}catch(e){return true;}}();
					if(isRoot) {
						var item = data[i];
						rootItems.push(item);
						data.remove(item);
				...
				var html = "";
				$.each(rootItems, function (i, item) {
					var childHtml = createHtmlForChildItems(item.id, parentField);
					var hasChild = childHtml.length > 0;
					html = html + "<li class=''><a " + (hasChild ? "href='#' class='dropdown-toggle'" : ("data-url='" + item.url + "' href='#" + item.url + "'")) + ">" +
					 "<i class='menu-icon fa fa-" + (item.iconCls && item.iconCls.length > 0 ? item.iconCls : "tachometer") + "'></i>" +
								"<span class='menu-text'> " + item.text + " </span>" + (hasChild ? "<b class='arrow fa fa-angle-down'></b>" : "") + "</a><b class='arrow'></b>" +
								childHtml + 
								"</li>";
		        });
				menubar.html(html + menubar.html());

				ace.demo.init();
				
				function createHtmlForChildItems(parentid, parentField) {
					var html = "";
					$.each(data, function (i, item) {
						var isRoot = function(){try{if(eval("data[i]." + parentField)){return false;} else return true;}catch(e){return true;}}();
						if(!isRoot && eval("item." + parentField) == parentid) {
							var childHtml = createHtmlForChildItems(item.id, parentField);
							var hasChild = childHtml.length > 0;
							html = html + "<li class=''><a " + (hasChild ? "href='#' class='dropdown-toggle'" : ("data-url='" + item.url + "' href='#" + item.url + "'")) + ">" +

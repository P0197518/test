			...
			.each(
					function() {
						if ($(this).attr('id') != undefined
								&& $(this).attr('id') != '') {
							mini.get($(this).attr('id')).setValue(
									$.formatMoneyAutoDigit(mini.get(
...
function concatDownloadUrl(url, data, sortField, sortOrder) {
	var urlData = window.encodeURIComponent(mini.encode(data));
	var condationStr = "";
...
function downloadExcel(url, data, sortField, sortOrder) {
	...
	if (!document.getElementById('fileDownloadFrame')) {
		...
		$(
				"<iframe id='fileDownloadFrame'style='display:none;' frameborder='no' ></iframe>")
				.prependTo('body');
	}
	document.getElementById("fileDownloadFrame").src = targetUrl + "&"
			+ (new Date()).valueOf();

	mini.mask({
		el : document.body,
		cls : 'mini-mask-loading',
		html : 'file download is started,please wait a moment...'
	});
